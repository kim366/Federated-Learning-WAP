...
%Kim
    ...
    \begin{frame} {Resource Utilization}
        ...
        \end{itemize}
    \end{frame}
    \begin{frame} {\texttt{FederatedAveraging} Algorithm (simplified)}
        \begin{center}
                ...
                \hskip 2em \textbf{for each} round $t = 1, 2, ...$ \textbf{do}\par
                    ...
                    \hskip 4em \textbf{for each} client $k \in S_t$ \textbf{in parallel do}\par
                        \hskip 6em $w_{t+1}^k \leftarrow$ ClientUpdate($k$, $w_t$)\par
                        \hskip 4em $w_{t+1} \leftarrow \sum_{k=1}^K \frac{n_k}{n} w_{t+1}^k$


                        \vskip 1em \emph{Client $k$}\par
                        \textbf{ClientUpdate}($k$, $w$) \textbf{do}\par
                    \hskip 2em \textbf{for each} local computation from 1 to $E$ \textbf{do}\par
                        \hskip 4em optimize $w$ with local dataset$\par
                    \hskip 2em \textbf{return} $w$ to server\par
                \end{minipage}
        \end{center}
    \end{frame}
    \begin{frame}[plain]
